...
async def main():
    ...
    await db.zvms.get_collection('logs').delete_many({
        'user': {'$in': users}
    })
    await db.zvms_new.get_collection('activities').update_many({
        ...
        {
            '$set': {
                'creator': '65e6fa210edc81d012ec46d9'
            }
        }
    )
    await db.zvms.get_collection('users').delete_many({
        'group': {'$in': groups}
    })
    await db.zvms.get_collection('groups').delete_many({
        'name': {'$regex': '三'}
    })
