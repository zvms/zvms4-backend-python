...
from utils import compulsory_temporary_token, get_current_user, randomString
import config
...
async def get_image(file_id: str, user=Depends(get_current_user)):
    if not user:
        raise HTTPException(status_code=401, detail="Unauthorized")
    image = image_storage.getBBImage(file_id)
...
async def delete_image(file_id: str, user=Depends(compulsory_temporary_token)):
    image = db.zvms.images.find_one({"id": file_id})
    if not image:
        raise HTTPException(status_code=404, detail="Image not found")
    if image["uploader"] != user["id"] and "admin" not in user["per"]:
        raise HTTPException(status_code=403, detail="Permission denied")

    image_storage.remove(file_id)
    await db.zvms.images.delete_one({"id": file_id})
